...
levels = [150, 200, 300, 350, 450, 550]
...
def upadate(bg_color, screen, stats, sc, gun, inos, bullets):
    """обновление экрана"""
...
def update_bullets(screen, stats, sc, inos, bullets):
    ...
    if len(inos) == 0:
        bullets.empty()
        create_army(screen, inos, True)


def gun_kill(stats, screen, sc, gun, inos, bullets):
    """столкновение пушки армии"""
    if stats.guns_left > 0:
        stats.guns_left -= 1
        sc.image_guns()
        inos.empty()
        bullets.empty()
        create_army(screen, inos, False)
        gun.create_gun()
        time.sleep(1)
    else:
...
def create_army(screen, inos, flag):
    ...
    number_ino_y = int((count - 2 * ino_height) / ino_height)
